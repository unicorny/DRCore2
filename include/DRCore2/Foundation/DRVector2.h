...
/*
Vector2D Klasse, uebernommen aus der TriBase-Engine von David Scherfgen
*/
class DRMatrix;

class DRCORE2_EXPORT DRVector2
{
public:
	//Variablen
	union
...
//    inline DRVector2	min(const DRVector2& v2)							   const {return DRVector2(DR_MIN(x, v2.x), DR_MIN(y, v2.y));}
 //   inline DRVector2	max(const DRVector2& v2)							   const {return DRVector2(DR_MAX(x, v2.x), DR_MAX(y, v2.y));}
    DRVector2	transformCoords(const DRMatrix& m, DRReal* const pfOutW = nullptr) const; // 2D-Vektor mit Matrix multiplizieren
